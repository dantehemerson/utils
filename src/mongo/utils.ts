...
function objectIDsEqual(a, b): boolean {
  if (a === b) return true
  if (!a) return !b
  if (!b) return false
...
function stringToObjectID(value: string): ObjectID {
  try {
...
function asObjectID(id): ObjectID {
  if (!id) {
    return null
  }
...
function asObjectIDs(ids: any[]): ObjectID[] {
  return ids.map(id => asObjectID(id))
}

function getDateFromObjectId(id): Date {
  if (!id) {
    return null
  }
...
function getTimestampFromObjectId(id): number {
...
function getNewObjectIdString(): string {
  var objectId = new ObjectID()

  return objectId.valueOf() as string
}

/**
 * Checks to see whether something is either a String or ObjectID (hence ObjectID-like)
 */
function isLikeObjectId(value): boolean {
...
function serializeObjectId(id): string {
  if (!id) return id
  if (typeof id === 'string') {
    return id
  }
...
function serializeObjectIds(ids): string[] {
  if (!ids || !ids.length) return []

  return ids.map(serializeObjectId)
}

function createIdForTimestampString(timestamp): string {
...
function createIdForTimestamp(timestamp): ObjectID {
  return new ObjectID(createIdForTimestampString(timestamp))
}

let currentIncrement = 0

function createTestIdForTimestampString(timestamp): string {
  var hexSeconds = Math.floor(timestamp / 1000).toString(16)
...
export {
  ...
  getNewObjectIdString,
  serializeObjectId,
  serializeObjectIds,
  createIdForTimestamp,
  createIdForTimestampString,
  createTestIdForTimestampString,
  fieldInPredicate,
  conjunctionIds,
  mongoErrorWithCode,
  unionModelsById
