...
function asObjectIDs(ids) {
    return ids.map(function (id) { return asObjectID(id); });
}
...
function isLikeObjectId(value) {
    ...
    if (!value)
        return false;
    if ((value && value._bsontype === 'ObjectID') || value instanceof mongodb_1.ObjectID) {
        return true;
    }
...
function createIdForTimestamp(timestamp) {
    return new mongodb_1.ObjectID(createIdForTimestampString(timestamp));
}
...
exports.createTestIdForTimestampString = createTestIdForTimestampString;
...
exports.unionModelsById = unionModelsById;
